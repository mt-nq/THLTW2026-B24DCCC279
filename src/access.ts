...
export default function access(initialState: IInitialState) {
	...
	console.log('ROLES:', initialState.currentUser?.realm_access?.roles);
	console.log('SCOPES:', scopes);

	return {
		accessFilter: (route: any) => {
			// route public → ai cũng vào
			if (!route?.maChucNang) return true;

			// GUEST chỉ cần login là vào
			if (isGuest) return true;

			// user thường → check scope
			return scopes.includes(route.maChucNang);
		},

		manyAccessFilter: (route: any) => {
			if (isGuest) return true;
			return route?.listChucNang?.some((role: string) => scopes.includes(role));
		},
	};
}
